...
class RaceClassRawDataTable {
    ...
    setData(newData) {
        ...
        this._datatable.rows.add(newData);
    ...
    _createInstance(e, aData) {
        
        // const race_RacersApi = new this.Race_RacersApi()
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row, meta) {
                        let badge = row.lastResult == null? "" :
                            row.lastResult.toString().indexOf('Error') > -1 ?
                            `<br/><span class="badge bg-outline-danger">${row.lastResult}</span>`:
                            row.lastResult.toString().indexOf('Warning') > -1 ?
                            `<br/><span class="badge bg-outline-warning">${row.lastResult}</span>` :
                            `<br/><span class="badge bg-outline-info">${row.lastResult}</span>`;
                        return `${row.Description} ${badge}` 
                    },
                },
...
export default RaceClassRawDataTable;
