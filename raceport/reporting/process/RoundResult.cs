...
namespace reporting.process
{
    public class RoundResult
    {
        ...
        public RoundResult(IUnitOfWork unitOfWork) {
            _unitOfWork = unitOfWork;
        }

        public string DoGenerate(string roundId)
        {
            //Process(stub);
            return RawDogIt(roundId);
        }
        ...
        //    ProcStubDto stub = await ProcStubClient.createStub("RoundResult");
       ...
       //     string filename = "RoundResult_" + UlidFormatter.ToString(Ulid.NewUlid().ToString()) + ".xls";
       //     try
       //     {
       ...
       // }
        ...
        private string RawDogIt(string roundId)
        {
            SpreadsheetGear.IWorkbook workbook;
            SpreadsheetGear.IWorksheet worksheet;
            ...
            render.Render("Round Result", "Master List", "List of all active race drivers", "", "");
            worksheet.Name = "MasterList";
            worksheet.Protect("1hellUvaK3y!");

            //List<Round> roundsList = _unitOfWork.Rounds.GetAll().ToList();
            Round round = _unitOfWork.Rounds.Find(roundId);
            ...
            if (round != null)
            {
                ...
                render = new ResultRender(worksheet, _unitOfWork);
                render.Render("Round Timesheet",
                    "Time Results -" + round.Name,
                    "Recorded race time results",
                    round.Id, round.Name);
                worksheet.Name = round.Name + " Results";
                worksheet.Protect("1hellUvaK3y!");
