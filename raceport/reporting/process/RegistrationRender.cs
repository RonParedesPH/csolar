...
{
    public class RegistrationRender : IWorksheetWorker
    {
        ...
        public RegistrationRender(IWorksheet worksheet, IUnitOfWork unitofwork) : base(worksheet, unitofwork)
        {
        ...
        public bool PrepareWorksheet(string key)
        {
            ...
            List<columnDefs> defsList = DefinedSetsOfColumnDefs.RoundSet().columnList;
            int offset = 0;
        ...
        public bool PrintContent(string key)
        {
            ...
            {
                foreach (columnDefs def in DefinedSetsOfColumnDefs.RoundSet().columnList)
                {
                    _assignCellValue(
                ...
                int len = DefinedSetsOfColumnDefs.RoundSet().columnList.Count;
                foreach (columnDefs def in DefinedSetsOfColumnDefs.RoundSet().columnList)
                {
                    _assignCellValue(
        ...
        private bool VerticalParameters()
        {
            ...
            for (int j = 0; j < max; j++)
            {
                ...
                col = 5;
                r = wsht.Cells[reportHelper.cell(col, j + i)];
                r.Formula = string.Format("= IF(LEN({0}) = 0, \"\", VLOOKUP({0}, MasterList!C$8:I$2008, 2, FALSE))",
                    reportHelper.cell(col - 2, j + i)
                    );
                ...
                r.Formula = string.Format("= IF(LEN({0}) = 0, \"\", VLOOKUP({0}, MasterList!C$8:J$2008, 8, FALSE))",
                    reportHelper.cell(col - 5, j + i)
                    );

                // Driver class
                col = 9;
                r = wsht.Cells[reportHelper.cell(col, j + i)];
                r.Formula = string.Format("= IF(LEN({0}) = 0, \"\", VLOOKUP({1}, MasterList!C$8:J$2008, 7, FALSE))",
                    reportHelper.cell(col - 5, j + i),
                    reportHelper.cell(col - 6, j + i)
                    );

                col = 10;
                r = wsht.Cells[reportHelper.cell(col, j + i)];
                r.Formula = string.Format("= IF(AND(LEN(B{0})>0,LEN(C{0})>0,LEN(D{0})>0),IF(VLOOKUP(D{0}&C{0},CQ$12:DG$2012,17,FALSE)=DG{0},\"\",\"D\"),\"\")",
                    (j + i).ToString()
                    );
                r.Font.Color = System.Drawing.Color.Red;
                ...
                col = _stash.racerHiddenOffset + 11;
                r = wsht.Cells[reportHelper.cell(col, j + i)];
                r.Formula = string.Format("={0}+1",
                    reportHelper.cell(_stash.racerHiddenOffset + 11, j + i -1));
        ...
        //List<columnDefs> _hiddenRoundColumns()
        //{
